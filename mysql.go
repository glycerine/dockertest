...
// or makes the test fail on error. The commandLineArgs can be nil and
// will be appended to the docker run command line. For example,
// commandLineArgs could be used to provide "--binlog-format=ROW";
// if that was required for testing on mysql.
//
func SetupMySQLContainer(commandLineArgs ...string) (c ContainerID, ip string, port int, err error) {
	port = RandomPort()
	...
	c, ip, err = SetupContainer(MySQLImageName, port, 10*time.Second, func() (string, error) {
		return run(append([]string{"--name", GenerateContainerID(), "-d", "-p", forward, "-e", fmt.Sprintf("MYSQL_ROOT_PASSWORD=%s", MySQLPassword), MySQLImageName}, commandLineArgs...)...)
	})
	return
}
...
func ConnectToMySQL(tries int, delay time.Duration, connector func(url string) bool, commandLineArgs ...string) (c ContainerID, err error) {
	c, ip, port, err := SetupMySQLContainer(commandLineArgs...)
	if err != nil {
